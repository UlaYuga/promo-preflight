...
- `GET /api/health`, `GET /api/ready` — liveness and readiness probes (the latter checks env, db, migrations).

### Persistence and pipeline

- PostgreSQL via Drizzle ORM + `pg`. Migrations under `db/migrations/` are applied on server boot from `instrumentation.node.ts` (Next standalone strips `pg`/`drizzle-orm` from `node_modules`, so a separate migrate script is not possible — T-057).
...
- Transactional outbox: domain events (`RunStarted`, `BlockerRaised`, `RunCompleted`) are inserted into `outbox` in the same transaction as the run. The outbox worker boots in-process from `instrumentation.node.ts` after migrations succeed (T-062), drains the queue with `FOR UPDATE SKIP LOCKED`, and delivers to two subscribers in order `[audit, telegram]` so the durable sink runs before the best-effort notifier (T-064).
...
- Quality gates per PR: typecheck, lint, schema check, db migrate, db check, vitest (162 tests / 23 files), rules check, owners check, i18n parity (447/447 leaf keys EN+RU), versioning check, AI check, checks regression smoke. Docker build runs in a separate job.
